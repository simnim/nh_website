...
async def test_real_imdb_percentiles_and_top_flag(tv_conn):
    episodes = [
        row async for row in QS.episodes.get_top_episodes_for_show(tv_conn, imdb_show_id=VOYAGER_ID, max_rank_pct=20)
    ]
...
async def test_real_imdb_seasons_summary(tv_conn):
    seasons = [row async for row in QS.episodes.get_seasons_summary(tv_conn, imdb_show_id=VOYAGER_ID)]
...
async def test_real_imdb_full_text_search(tv_conn, monkeypatch):
    ...
    ids = await main.get_search_results_given_search_str("trek voyager", return_just_id=True)
...
async def test_real_top_cat_posts_for_hash(cat_conn):
    ...
    posts = [row async for row in QS.topcat.get_posts_for_hash(cat_conn, media_hash=media_hash, ts_ins=None)]
    assert len(posts) > 0
    assert all(p["media_hash"] == media_hash for p in posts)


async def test_real_top_cat_recent_posts(cat_conn):
    posts = [row async for row in QS.topcat.get_top_posts_for_flask(cat_conn, label="cat")]
...
async def test_real_books(books_conn):
    categories = [row["category"] async for row in QS.books.get_categories_for_top_books(books_conn)]
    assert categories
    books = [row async for row in QS.books.get_top_books_for_category(books_conn, category=categories[0])]
